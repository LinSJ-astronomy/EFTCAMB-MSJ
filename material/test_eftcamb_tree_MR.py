...
def generate_eft_tree(eftcamb_params, tree, feedback=0, depth=0, max_depth=1, max_int=20):
    ...
    name, input_params, diff_params = check_params(eftcamb_params)
    # feedback:
    if feedback > 0:
        print('*************************************')
...
tree = eftcamb_tree()
# generate tree:
generate_eft_tree(eftcamb_params={}, feedback=1, tree=tree, max_depth=10)
# consolidate tree:
consolidate_tree_properties(tree)
# all keys:
...
# tree depth:
print('Tree depth:', get_max_depth(tree))

############################################################


temp_tree = tree.children[1]

temp_tree.in_params
temp_tree.out_params
temp_tree.numerical_params



[[tt.changed_param, len(tt.children)] for tt in temp_tree.children]
...
"""
Eliminate all redundant flags. i.e. feedback is varied everywhere -> add it as a parameter only to the base leaf.
"""

def test(tree):
    """
    Print a primitive flowchart
    """
    depth = tree.depth
    whitespaces = '  ' * depth
    # print depth info:
    if depth == 0:
        ...
        print(whitespaces, '└---------')
    print(whitespaces, 'Depth:', tree.depth)
    # print model name:
    if tree.name != '':
        print(whitespaces, tree.name)
    # print changed parameters:
    if tree.changed_param is not None:
        print(whitespaces, tree.changed_param, '=', tree.in_params[tree.changed_param])
    # go down the tree:
    for child in tree.children:
        test(child)
...
pippo = tree.children[4].children[-2]
